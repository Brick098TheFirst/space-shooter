...
- **Cheat Codes (Settings → CODES):** A CODES row in Settings opens a native Android text dialog. `GIMMEMONEY` grants **999 trillion coins** ($999,000,000,000,000). Coins are 64-bit on Android (`coin_t`), and the save blob is written in a new host-only **V6** layout (legacy V5 saves migrate automatically). Big balances print shortened (e.g. `999T`, `1.5B`).
