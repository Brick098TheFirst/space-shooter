...
class GbaPlayer {
    ...
    initAudio() {
        ...
        this.audioNode.onaudioprocess = (e) => {
            ...
            for (let i = 0; i < bufferSize; i++) {
                if (this.audioRingBuffer.length >= 2) {
                    const sampleL = this.audioRingBuffer.shift();
                    const sampleR = this.audioRingBuffer.shift();
                    outL[i] = (sampleL / 32768.0) * effectiveVol;
                    outR[i] = (sampleR / 32768.0) * effectiveVol;
                } else {
                    outL[i] = 0;
                    outR[i] = 0;
                }
    ...
    async loadRom(romBuffer) {
        ...
        const audioCb = m.addFunction((dataPtr, frames) => {
            if (this.audioRingBuffer.length > this.audioBufferSize) {
                return frames; // Drop to avoid high latency
            }
            const src16 = new Int16Array(m.HEAPU8.buffer, dataPtr, frames * 2);
            for (let i = 0; i < src16.length; i++) {
                this.audioRingBuffer.push(src16[i]);
            }
            return frames;
        }, "iii");
