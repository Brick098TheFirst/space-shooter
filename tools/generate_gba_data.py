...
# Hull-style templates live in tools/preview_ship_styles.py (the art source
# of truth - it also renders the PNG previews).  Import the pixel pipeline so
# the regenerated gfx_data.c/.h always carry spr_ship_styles and stay in sync
# with renderer.c's gfx_draw_ship_styled().
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__))))
import preview_ship_styles as _pss
...
def build_ship_styles():
    """Compute the spr_ship_styles table exactly like preview_ship_styles.py."""
    classic = _pss.parse_classic()

    # Style 0: faithful transcription of the classic paint-0 sprite with the
    # accent pixels remapped to the 241..243 mask range.
    cyber = []
    for p in classic[0]:
        if p == 49:
            cyber.append(241)
    ...
    styles = [
        ("CYBER", cyber),
        ("VIPER", _pss.rows_to_pixels(_pss.mirror(_pss.VIPER), "VIPER")),
        ("MANTA", _pss.rows_to_pixels(_pss.mirror(_pss.MANTA), "MANTA")),
        ("TITAN", _pss.rows_to_pixels(_pss.mirror(_pss.TITAN), "TITAN")),
        ("PHOENIX", _pss.rows_to_pixels(_pss.mirror(_pss.PHOENIX), "PHOENIX")),
    ]
    return styles


ship_styles = build_ship_styles()
...
""")
    ...
    # Hull style templates (SHIPS shop tab).  Kept in sync with renderer.c's
    # gfx_draw_ship_styled(); the art lives in tools/preview_ship_styles.py.
    f.write("""/* -------------------------------------------------------------------------
 * Hull STYLE templates for the SHIPS shop tab (20x16, same palette language
 * as spr_ship: hull ramp 33..38, mask 241..243 -> accent ramp at runtime so
 * every paint including rainbow works on every hull).  Index 0 (CYBER) is a
 * faithful transcription of the classic spr_ship so previews stay identical;
 * gameplay still uses the spr_ship fast path for style 0.
 * Generated by tools/preview_ship_styles.py - edit the art there.
 * ------------------------------------------------------------------------- */
const u8 spr_ship_styles[NUM_SHIP_STYLES][20 * 16] = {
""")
    for name, pixels in ship_styles:
        f.write(f"    /* {name} */\n")
        f.write("    {\n")
        for y in range(_pss.H):
            row = pixels[y * _pss.W:(y + 1) * _pss.W]
